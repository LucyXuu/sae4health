...
mod_indicator_in_app_server <- function(id,CountryInfo,AnalysisInfo,MetaInfo,parent_session){
  moduleServer( id, function(input, output, session){
    ...
    output$selected_ind_tab <- DT::renderDataTable({
      ...
      selected_ind_table[selected_ind_table$ID == "CM_ECMR_C_NNR", ]$`Full_definition` <- "Deaths per 1000 live births in the first month of life in the five years preceding the survey. Estimates are given for ten year periods for all characteristics, but for five year periods only for the national total, by residence, and by sex."
